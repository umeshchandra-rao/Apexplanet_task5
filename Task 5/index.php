...
<div>
  <div class="nav-inner" style="margin:12px 0 16px;">
    ...
    <?php if (is_logged_in()) { ?>
      ...
      <span class="muted">Logged in as <?php echo htmlspecialchars($cu['username']); ?> (<?php echo htmlspecialchars($cu['role']); ?>)</span>
    <?php } else { ?>
    ...
    <?php foreach ($posts as $post) { ?>
      <div class="card">
        ...
        <div style="margin-top:12px; display:flex; gap:8px;">
          <?php if (can_access_resource('edit', $post)) { ?>
            <a href="edit.php?id=<?php echo (int)$post['id']; ?>" class="btn">Edit</a>
          <?php } ?>
          <?php if (can_access_resource('delete', $post)) { ?>
